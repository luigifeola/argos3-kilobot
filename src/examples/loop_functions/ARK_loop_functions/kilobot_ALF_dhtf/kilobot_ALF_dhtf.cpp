...
void CALFClientServer::Init(TConfigurationNode &t_node)
{
    ...
    if (mode == "SERVER")
    {
        ...
        outputBuffer = "I"; //fill the init message server -> client
        /* Select areas */
        srand(random_seed);
        int t = 0;
        ...
        for (int i = 0; i < num_of_areas; i++)
        {
            ...
            { //è una prova, non serve realmente
...
void CALFClientServer::UpdateKilobotState(CKilobotEntity &c_kilobot_entity)
{
    ...
    if (mode == "CLIENT")
    {
        ...
        if ((storeBuffer[0] == 73) && (initializing == true))
        ...
        if ((storeBuffer[0] == 65) && (initializing == false))
    ...
    if (mode == "SERVER")
    {
        ...
        if (arena_update_counter == 0)
        {
            ...
            for (int a = 0; a < num_of_areas; a++)
            {
                ...
                {
                    double r = ((double)rand() / (RAND_MAX));
                    if (r < 0.1) //remove this activation rate
                    {
                        multiArea[a].Completed = false;
                        contained[a] = 0;
                    }
