...
res_dir=$wdir/"results_dhtf/standalone_newWalkParam/adaptiveTimeout"
if [[ ! -e $res_dir ]]; then
...
param_dir=$res_dir/"Regions_Adaptive_"$date_time"_robots#"$numrobots"_timeout#"$timeout"_respawn#"$reactivation_timer"_NumAreas#"$desired_num_of_areas"_redAreas#"$hard_tasks"_"$experiment_length"#seconds"
...
for it in $(seq 1 $RUNS); do
    ...
    echo "Running next configuration seed $it Robots $numrobots TIMEOUT $timeout"
