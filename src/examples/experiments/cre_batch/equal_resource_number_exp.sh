...
# ./src/examples/experiments/cre_batch/equal_resource_number_exp.sh /src/examples/experiments/cre_batch cres_experiment.argos crec_experiment.argos

if [ "$#" -ne 3 ]; then
    echo "Usage: equal_resource_number_exp.sh (from src folder) <base_config_dir> <base_config_file_name_server> <base_config_file_name_client>"
    exit 11
fi
...
res_dir=$wdir/"results_cre/equal_resources_number"
if [[ ! -e $res_dir ]]; then
...
param_dir=$res_dir/"different_resources_"$date_time"_seconds#"$experiment_length
if [[ ! -e $param_dir ]]; then
